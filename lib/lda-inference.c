...
double alhood(double a, double ss, int D, int K) {
    double tmp = D * lgamma(K *a) - K * lgamma(a)) + (a - 1) * ss;
    return tmp;
    //return((double)(D * (lgamma(K * a) - K * lgamma(a)) + (a - 1) * ss));
}

double d_alhood(double a, double ss, int D, int K) {
  return((double)(D * (K * digamma(K * a) - K * digamma(a)) + ss));
}

double d2_alhood(double a, int D, int K) {
  return ((double)(D * (K * K * trigamma(K * a) - K * trigamma(a))));
}
