...
EXT_SHARED_OBJECTS = %w[so bundle dylib dll].map { |ext| File.join(EXT_DIR, "lda.#{ext}") }.freeze
...
CLOBBER.include(EXT_MAKEFILE, File.join(EXT_DIR, "mkmf.log"), *EXT_SHARED_OBJECTS)
